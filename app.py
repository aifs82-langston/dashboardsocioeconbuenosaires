...
def generar_pdf_completo(conteo_sexo, total, pct_ind, lista_graficos, titulos_graficos):
    """Genera un PDF profesional eliminando la redundancia de títulos en los gráficos."""
    pdf = FPDF()
    ...
    pdf.cell(200, 10, txt="Municipalidad de Buenos Aires, Costa Rica - 2026", ln=True, align='C')
    ...
    pdf.cell(200, 8, txt=f"Identificacion Indigena: {pct_ind:.1f}% de la muestra", ln=True)
    ...
    pdf.cell(200, 10, txt="Visualizaciones Detalladas", ln=True, align='L')
    pdf.line(10, pdf.get_y(), 200, pdf.get_y()) # Línea divisoria decorativa
    
    # Insertar Gráficos
    for i, fig in enumerate(lista_graficos):
        # 1. Escribir el título con la tipografía del PDF para consistencia
        pdf.ln(8)
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(200, 10, txt=f"{titulos_graficos[i]}", ln=True)
        
        # 2. PROCESO DE LIMPIEZA: Quitamos el título de la figura antes de guardar para evitar repetición
        # Guardamos el título original para restaurarlo después si es necesario en el dashboard
        ax = fig.gca()
        ...
        # Restaurar el título para que el dashboard no lo pierda si el usuario sigue navegando
...
if df is not None:
    ...
    if btn_analisis:
        with st.spinner('Procesando reporte completo...'):
            ...
            st.download_button(
                label="📥 Descargar Reporte PDF Final (Sin Repeticiones)",
                data=pdf_bytes,
